...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (enableBottom) {
    for (int i = 0; i < bottomLineCount; ++i) {
      float fi = float(i);
      float t = bottomLineCount > 1 ? fi / float(bottomLineCount - 1) : 0.0;
  ...
  if (enableMiddle) {
    for (int i = 0; i < middleLineCount; ++i) {
      float fi = float(i);
      float t = middleLineCount > 1 ? fi / float(middleLineCount - 1) : 0.0;
      vec3 lineCol = getLineColor(t, b);
  ...
  if (enableTop) {
    for (int i = 0; i < topLineCount; ++i) {
      float fi = float(i);
      float t = topLineCount > 1 ? fi / float(topLineCount - 1) : 0.0;
      vec3 lineCol = getLineColor(t, b);
...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    // Use an opaque white background by default to blend properly with images
    const renderer = new WebGLRenderer({ antialias: true, alpha: false });
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 2));
    renderer.setClearColor(0xffffff, 1); // clear white
