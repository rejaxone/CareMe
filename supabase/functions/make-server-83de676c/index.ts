...
function getMayarHeaders() {
  const apiKey = Deno.env.get("MAYAR_API_KEY") || "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..n5GnNfNAu44Pgvpack3bQwlz3rXMPrfM_43mTCfhG6mtPqkkIKzYhWDWXOageLx2N8STdi_WCD4bAfu9felaDAKy7_6vurx-KcIyu7U7GLJ_LofGa8DnXvB0SFj9K_rBLseio1wxCKZencpap4cueZEmFK1vATjQlM8BFe0fT7aPRo9jxHitJQ8FzfRQoiYSYYxYXs9dDJNqSXKbg_25soc7ZGHlxKkOm-5fmZI-qJFxdCbtp1_F7NY1fQFBK4odcJ79UO7LOPdAvQDOO61yvY2t82aIvGkGE1TMX91cP6MFGUztAFqYqEgGMwo9x8I0vPlMO4rG6jCwJoKCLohD-A";
...
app.post("/make-server-83de676c/api/payments/create", async (c) => {
  try {
    ...
    // Direct payment link as requested
    const paymentLink = "https://mayar.to/galaxy-guardian";
    const paymentId = `PAY-${Date.now()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;

    // Update booking directly to paid
    ...
    const payment = {
      ...
      mayarTransactionId: "auto-paid",
      mayarResponse: { message: "Auto marked as paid based on user click" },
      createdAt: new Date().toISOString(),
    ...
    console.log("POST /api/payments/create: Auto-paid payment created", paymentId, "→", paymentLink);
...
app.post("/make-server-83de676c/api/payments/webhook", async (c) => {
  try {
    ...
    const webhookToken = Deno.env.get("MAYAR_WEBHOOK_TOKEN") || "69cf546842f44dbfb14a1b6ad4e1b0ec59e1dfbbf5b818617b0ba63d45435b5c1283471116911e9a384b72e1a71434d77cc7b5e92b6c5aeffeeddcac1b9c1b54";
    const headerToken =
    ...
    const paidAmount = data.amount || data.paidAmount || data.net_amount || 0;
    
    // Mayar specific reference
    const referenceId = data.reference_id || data.referenceId || (data.metadata ? data.metadata.bookingId : null);

    console.log("Webhook event:", event, "| transactionId:", transactionId, "| status:", status, "| referenceId:", referenceId);

    // 1. Find the booking by referenceId (Fastest and most reliable approach)
    let matchedBooking = null;
    let matchedPayment = null;

    if (referenceId) {
      matchedBooking = await kv.get(`booking:${referenceId}`);
      if (matchedBooking && matchedBooking.paymentId) {
         matchedPayment = await kv.get(`payment:${matchedBooking.paymentId}`);
      }
    }

    // 2. If not found by referenceId, fallback: search through payments by transactionId
    if (!matchedBooking) {
    ...
    // 3. Second fallback: try matching by description or booking ID embedded in the payload
