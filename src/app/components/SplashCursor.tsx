...
export default function SplashCursor({
  ...
  // CareMe theme: use the brand blue (0x2E8BFF → r:0.18, g:0.55, b:1.0) as base tint
  BACK_COLOR = { r: 0.0, g: 0.05, b: 0.15 },
  TRANSPARENT = true
}: SplashCursorProps) {
  ...
  useEffect(() => {
    ...
    class Material {
      ...
      uniforms: Record<string, WebGLUniformLocation | null>;
      constructor(vs: WebGLShader | null, fss: string) {
    ...
    const curlShader = compileShader(gl.FRAGMENT_SHADER, `
      ...
      uniform sampler2D uVelocity;
      void main () {
        float vorticity = texture2D(uVelocity, vR).y - texture2D(uVelocity, vL).y - texture2D(uVelocity, vT).x + texture2D(uVelocity, vB).x;
    ...
    const pressureShader = compileShader(gl.FRAGMENT_SHADER, `
      ...
      void main () {
        float pressure = (texture2D(uPressure, vL).x + texture2D(uPressure, vR).x + texture2D(uPressure, vB).x + texture2D(uPressure, vT).x - texture2D(uDivergence, vUv).x) * 0.25;
    ...
    const gradientSubtractShader = compileShader(gl.FRAGMENT_SHADER, `
      ...
      void main () {
        ...
        velocity.xy -= vec2(texture2D(uPressure, vR).x - texture2D(uPressure, vL).x, texture2D(uPressure, vT).x - texture2D(uPressure, vB).x);
        gl_FragColor = vec4(velocity, 0.0, 1.0);
      }`);
    ...
    // CareMe theme: generate blue-toned colors (hue 190–240 = cyan to blue range)
    function generateColor(): ColorRGB {
      // Bias hue toward CareMe blue (around 210°)
      const hue = 0.52 + Math.random() * 0.15; // 187–241 degrees
    ...
    function correctRadius(radius: number) {
      const ar = canvas!.width / canvas!.height; if (ar > 1) radius *= ar; return radius;
    }
    ...
    // Start render loop on first mouse/touch interaction
